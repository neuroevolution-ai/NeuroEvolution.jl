...
function kernel_eval_fitness(individuals,results, env_seed,number_rounds,brain_cfg::CTRNN_Cfg,environment_cfg::Collect_Points_Env_Cfg)
    ...
    for j in 1:number_rounds
        if threadIdx().x == 1
            Random.seed!(Random.default_rng(),env_seed+j)
        end
        sync_threads()
        #reset brain
        @inbounds x[tx] = 0.0f0
...
function kernel_eval_validation(individuals,results,brain_cfg::CTRNN_Cfg,environment_cfg::Collect_Points_Env_Cfg)
    tx = threadIdx().x
    fitness_total = 0
    ...
    sync_threads()
        ...
        if threadIdx().x == 1
            Random.seed!(Random.default_rng(),blockIdx().x+1)
        end
        sync_threads()
        #reset brain
        @inbounds x[tx] = 0.0f0
    ...
    #end of Round
    if tx == 1
        @inbounds results[blockIdx().x] = fitness_total
    end
    sync_threads()
    return
end

...
function kernel_eval_fitness(individuals,results, env_seed,number_rounds,brain_cfg::CTRNN_Cfg,environment_cfg::Collect_Points_Env_Cfg)
    ...
    total_amount_of_cells = environment_cfg.maze_columns * environment_cfg.maze_rows
    screen_width = environment_cfg.maze_cell_size * environment_cfg.maze_columns
    screen_height = environment_cfg.maze_cell_size * environment_cfg.maze_rows
    ...
    for j in 1:number_rounds
        ...
        #agent_x_coordinate = convert(Int32,(abs(rand(Int32)) % (environment_cfg.maze_cell_size - (2*environment_cfg.agent_radius))) + environment_cfg.agent_radius +((abs(rand(Int32)) % environment_cfg.maze_columns)) * environment_cfg.maze_cell_size)

        #agent_y_coordinate = convert(Int32,(abs(rand(Int32)) % (environment_cfg.maze_cell_size - (2*environment_cfg.agent_radius))) + environment_cfg.agent_radius +((abs(rand(Int32)) % environment_cfg.maze_rows)) * environment_cfg.maze_cell_size)
        ...
        if tx == 1 || tx == 3 || tx == 5 
            @inbounds environment_config_array[tx] = convert(Int32,(abs(rand(Int32)) % (environment_cfg.maze_cell_size - (2*environment_cfg.agent_radius))) + environment_cfg.agent_radius +((abs(rand(Int32)) % environment_cfg.maze_columns)) * environment_cfg.maze_cell_size)
        end
        if tx == 2 || tx == 4 || tx == 6 
            @inbounds environment_config_array[tx] = convert(Int32,(abs(rand(Int32)) % (environment_cfg.maze_cell_size - (2*environment_cfg.agent_radius))) + environment_cfg.agent_radius +((abs(rand(Int32)) % environment_cfg.maze_rows)) * environment_cfg.maze_cell_size)#
        end
        sync_threads()
        if tx==1

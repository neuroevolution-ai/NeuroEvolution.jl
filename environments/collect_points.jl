...
function kernel_eval_fitness(individuals, rewards, environment_seeds, number_rounds, brains,environments::CollectPoints)
    ...
    observations =
        @cuDynamicSharedMem(Float32, environments.number_outputs, offset_shared_memory)
    ...
    sensor_distances =
        @cuDynamicSharedMem(Float32, environments.number_sensors, offset_shared_memory)
    offset_shared_memory += sizeof(sensor_distances)

    sync_threads()

    ray_directions =
        @cuDynamicSharedMem(Float32, (2, environments.number_sensors), offset_shared_memory)
    ...
    offset_shared_memory +=
        sizeof(Int32) * (environments.maze_columns * environments.maze_rows * 2) +
        sizeof(Int32) * 8
...
function initialize(threadID, blockID, sensor_distances, ray_directions, ray_cell_distances, environments::CollectPoints, offset, rng_states)
    ...
    #Initialize the Ray directions & cell_distances
...
function step(threadID, blockID, action, observations, sensor_distances, ray_directions, ray_cell_distances, rewards, environments::CollectPoints, rng_states,)
    ...
    if threadID == 1
        ...
        # Move agent
        environments.agents_positions[1, blockID] +=
            convert(Int, round(clamp(action[1] * move_range, -move_range, move_range)))
        environments.agents_positions[2, blockID] +=
            convert(Int, round(clamp(action[2] * move_range, -move_range, move_range)))

        #Check Collision outer borders of the maze
        ...
        if (environments.agents_positions[1, blockID] - cell_left < environments.agent_radius 
        ...
        if (cell_right - environments.agents_positions[1, blockID] < environments.agent_radius 
            & environments.agents_positions[2, blockID] - cell_top < environments.agent_radius)
            environments.agents_positions[1, blockID] = cell_right - environments.agent_radius
            environments.agents_positions[2, blockID] = cell_top + environments.agent_radius
        end
        if (cell_right - environments.agents_positions[1, blockID] < environments.agent_radius 
            & cell_bottom - environments.agents_positions[2, blockID] < environments.agent_radius)
            environments.agents_positions[1, blockID] = cell_right - environments.agent_radius
        ...
        distance = sqrt(
            (environments.positive_points_positions[1, blockID] - environments.agents_positions[1, blockID])^2 +
            (environments.positive_points_positions[2, blockID] - environments.agents_positions[2, blockID])^2,)
        if distance <= environments.point_radius + environments.agent_radius
            environments.positive_points_positions[1, blockID],
            environments.positive_points_positions[2, blockID] =
                place_randomly_in_maze(blockID, environments, rng_states)
            reward += environments.reward_per_collected_positive_point
        end

        # Collect negative point in reach
        distance = sqrt(
            (environments.negative_points_positions[1, blockID] - environments.agents_positions[1, blockID])^2 +
            (environments.negative_points_positions[2, blockID] - environments.agents_positions[2, blockID])^2,
        )
        if distance <= environments.point_radius + environments.agent_radius
            environments.negative_points_positions[1, blockID],
            environments.negative_points_positions[2, blockID] =
                place_randomly_in_maze(blockID, environments, rng_states)
...
function create_maze(threadID, blockID, environments::CollectPoints, offset, rng_states)
    ...
    if threadID == 1
        ...
        while nv < total_amount_of_cells
            ...
            number_neighbours = find_valid_neighbors(
                blockID,
                current_cell_x,
                current_cell_y,
                neighbours,
...
function init_ray(sensor_number, init_x, init_y, angle_diff_per_ray, ray_directions, ray_cell_distances, environments::CollectPoints)
    ...
    #Thread check
    if sensor_number <= environments.number_sensors
        ...
        if -1e-10 < ray_directions[1, sensor_number] < 1e-10
            ...
            ray_cell_distances[1, sensor_number] = sqrt(
                1 +
                (ray_directions[2, sensor_number] / ray_directions[1, sensor_number]) *
                (ray_directions[2, sensor_number] / ray_directions[1, sensor_number]),
            )
        end
        ...
        if -1e-10 < ray_directions[2, sensor_number] < 1e-10
            ...
            ray_cell_distances[2, sensor_number] = sqrt(
                1 +
                (ray_directions[1, sensor_number] / ray_directions[2, sensor_number]) *
                (ray_directions[1, sensor_number] / ray_directions[2, sensor_number]),
            )
        end
...
function calculate_ray_distance(sensor_number, individual, sensor_distances, ray_directions, ray_cell_distances, environments::CollectPoints,)
    if sensor_number <= environments.number_sensors
        current_cell_x = convert(
            Int,
            ceil(environments.agents_positions[1, individual] / environments.maze_cell_size),
        )
        current_cell_y = convert(
            Int,
            ceil(environments.agents_positions[2, individual] / environments.maze_cell_size),
        )

        cell_left, cell_right, cell_top, cell_bottom = get_coordinates_maze_cell(
            current_cell_x,
            current_cell_y,
            environments.maze_cell_size,
        )

        if ray_directions[1, sensor_number] < 0.0
            step_direction_x = -1
            ray_length_x =
                (environments.agents_positions[1, individual] - cell_left) /
                environments.maze_cell_size
        ...
        if ray_directions[2, sensor_number] < 0.0
            ...
            ray_length_y =
                1 - (
                    (environments.agents_positions[2, individual] - cell_top) /
                    environments.maze_cell_size
                )
        ...
        while !hit
            ...
            if maze_walls[current_cell_x, current_cell_y, individual]
                ...
                side == 1 ? current_cell_x += step_direction_x :
                current_cell_y += step_direction_y
            end
        end
...
function place_randomly_in_maze(blockID, environments::CollectPoints, rng_states)
    ...
    x_position =
        lgc_random(rng_states, 1, environments.maze_rows - 1) * environments.maze_cell_size
    x_position += lgc_random(
        rng_states,
        1,
        environments.agent_radius,
        (environments.maze_cell_size - environments.agent_radius),
    )

    y_position =
        lgc_random(rng_states, 1, environments.maze_columns - 1) *
        environments.maze_cell_size
    y_position += lgc_random(
        rng_states,
        1,
        environments.agent_radius,
        (environments.maze_cell_size - environments.agent_radius),
...
function get_memory_requirements(environments::CollectPoints)
    ...
    return sizeof(Int32) * (2 * total_amount_of_cells) +
           sizeof(Int32) * 8                           +                       #Maze initialization
...
function render(sensor_distances, environments::CollectPoints, individual)
    ...
    circle(
        positive_points_position[1],
        positive_points_position[2],
        environments.point_radius,
        :fill,
    )
    ...
    circle(
        negative_points_position[1],
        negative_points_position[2],
        environments.point_radius,
        :fill,
    )
    ...
    for i = 1:environments.number_sensors
        ...
        agent_point = Point(
            agent_position[1] + environments.agent_radius * cos(angle_diff),
            agent_position[2] - environments.agent_radius * sin(angle_diff),
        )
        colission_point = Point(
            agent_position[1] +
            environments.agent_radius * cos(angle_diff) +
            sensor_distances[i] * cos(angle_diff),
            agent_position[2] - environments.agent_radius * sin(angle_diff) -
            sensor_distances[i] * sin(angle_diff),
        )
    ...
    for cell_x = 1:environments.maze_columns
        for cell_y = 1:environments.maze_rows
            ...
            cell_left, cell_right, cell_top, cell_bottom =
                get_coordinates_maze_cell(cell_x, cell_y, environments.maze_cell_size)
